...
export function reusableBarChart() {
  const initialConfiguration = {
    ...
    tooltipFormatter: (d) => `${d.data.label}: ${d.data.value}`,
  };

  let { width } = initialConfiguration;
  let { height } = initialConfiguration;
  let { padding } = initialConfiguration;
  let { data } = initialConfiguration;
  const { colorScale } = initialConfiguration;
  let { tooltipFormatter } = initialConfiguration;
  let updateData; let
    xScale = null;
  ...
  function chart(selection) {
    selection.each(() => {
      ...
      const tooltip = d3Tip()
        .attr('class', 'd3-tip')
        .direction((d) => (d.isArea ? 's' : 'n'))
        .offset((d) => (d.isArea ? [8, 0] : [-8, 0]))
